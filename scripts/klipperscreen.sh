...
function install_Aiklipperscreen() {
  ### return early if python version check fails
  if [[ $(python3_check) == "false" ]]; then
    local error="Versioncheck failed! Python 3.7 or newer required!\n"
    error="${error} Please upgrade Python."
  ...
  #print_confirm " Çalıştı"

  ### first, we create a backup of the full klipper_config dir - safety first!
  backup_klipper_config_dir

  ### install KlipperScreen
  Aiklipperscreen_setup
...
function Aiklipperscreen_setup() {
  local dep=(wget curl unzip dfu-util)
  dependency_check "${dep[@]}"
  status_msg "Cloning KlipperScreen from ${AiKLIPPERSCREEN_REPO} ..."

  # force remove existing KlipperScreen dir
  [[ -d ${AiKLIPPERSCREEN_DIR} ]] && rm -rf "${AiKLIPPERSCREEN_DIR}"

  # clone into fresh KlipperScreen dir
  cd "${HOME}" || exit 1
  if ! git clone "${AiKLIPPERSCREEN_REPO}" "${AiKLIPPERSCREEN_DIR}"; then
    print_error "Cloning KlipperScreen from\n ${AiKLIPPERSCREEN_REPO}\n failed!"
    exit 1
  fi

  status_msg "Installing KlipperScreen ..."
  if "${AiKLIPPERSCREEN_DIR}"/scripts/KlipperScreen-install.sh; then
    ok_msg "KlipperScreen successfully installed!"
  else
    print_error "KlipperScreen installation failed!"
    exit 1
...
#===================================================#
#=============== REMOVE AIKLIPPERSCREEN ==============#
#===================================================#

function remove_Aiklipperscreen() {
  ### remove Ai KlipperScreen dir
  if [[ -d ${AiKLIPPERSCREEN_DIR} ]]; then
    status_msg "Removing AiKlipperScreen directory ..."
    rm -rf "${AiKLIPPERSCREEN_DIR}" && ok_msg "Directory removed!"
  fi

  ### remove Ai KlipperScreen VENV dir
  if [[ -d ${AiKLIPPERSCREEN_ENV} ]]; then
    status_msg "Removing AiKlipperScreen VENV directory ..."
    rm -rf "${AiKLIPPERSCREEN_ENV}" && ok_msg "Directory removed!"
  fi

  ### remove Ai KlipperScreen service
  if [[ -e "${SYSTEMD}/KlipperScreen.service" ]]; then
    ...
    ok_msg "AiKlipperScreen Service removed!"
  fi
...
#===================================================#
#=============== AiKLIPPERSCREEN STATUS ==============#
#===================================================#

function get_Aiklipperscreen_status() {
  local sf_count status
  sf_count="$(klipperscreen_systemd | wc -w)"

  ### remove the "SERVICE" entry from the data array if a moonraker service is installed
  local data_arr=(SERVICE "${AiKLIPPERSCREEN_DIR}" "${AiKLIPPERSCREEN_ENV}")
  (( sf_count > 0 )) && unset "data_arr[0]"
